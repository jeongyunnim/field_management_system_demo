...
export default function StartInspectionButton({
  ...
  className = "",
  disabled = false,
  onBegan, // 시작되면 부모에 알림 (isInspecting=true)
}) {
  const [hovering, setHovering] = useState(false);
  const [toast, setToast] = useState({ visible: false, text: "" });
  const toastTimerRef = useRef(null);
    ...
    <>
      <div className="relative inline-block">
        <button
          ...
          className={`relative flex items-center justify-center select-none rounded-lg font-medium transition
            ${widthClass} ${heightClass}
            ${disabled ? "bg-green-300 text-white cursor-not-allowed opacity-70" : "bg-green-600 text-white hover:bg-green-700"}
            ${className}`}
          aria-disabled={disabled}
        >
          <span
            className={`absolute left-2 top-1/2 -translate-y-1/2 rounded-full w-2.5 h-2.5
              ${disabled ? "bg-white/50" : hovering ? "bg-white" : "bg-white/80"}`}
          />
          <span className="pointer-events-none mx-auto whitespace-nowrap text-sm text-center overflow-hidden text-ellipsis">
            점검 시작
          </span>
