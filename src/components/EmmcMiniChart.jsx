// src/components/EmmcMiniChart.jsx
...
export default function EmmcMiniChart({ usageHistory, currentUsage }) {
  const data = {
  ...
  const options = {
    ...
    plugins: {
      ...
      tooltip: {
        enabled: true,
        callbacks: {
          label: (ctx) => `eMMC: ${ctx.raw.toFixed(1)}%`
        }
      }
    },
    animation: false
  ...
  return (
    <div className="flex items-center space-x-3 bg-white p-2 border border-gray-200 rounded shadow-sm w-fit">
      ...
      <div className="text-sm text-gray-800 leading-tight">
        <div className="font-semibold">eMMC</div>
