// src/components/RamMiniChart.jsx
...
export default function RamMiniChart({ usageHistory, currentUsage }) {
  const data = {
    ...
    datasets: [
      {
        ...
        backgroundColor: "rgba(16, 185, 129, 0.15)", // 연한 초록 배경
        fill: true,
        tension: 0.3,
        pointRadius: 0,
        borderWidth: 1.5
  ...
  const options = {
    ...
    plugins: {
      ...
      tooltip: {
        enabled: true,
        callbacks: {
          label: (ctx) => `RAM: ${ctx.raw.toFixed(1)}%`
        }
      }
    },
    animation: false
